...
if (!$db->isServiceAvailable('order') || !$db->isServiceAvailable('product')) {
   $message[] = 'Dịch vụ đặt hàng tạm thời không khả dụng';
} else {
   try {
      $order_db = $db->getConnection('order');
      $product_db = $db->getConnection('product');
      ...
      // Get cart items
      $select_cart = $product_db->prepare("SELECT * FROM `cart` WHERE user_id = ?");
...
<body>
   ...
   <section class="checkout">
      ...
      <?php if ($db->isServiceAvailable('order') && $db->isServiceAvailable('product')): ?>
      ...
      <?php else: ?>
         <div class="notice">
            <p>Dịch vụ đặt hàng tạm thời không khả dụng</p>
