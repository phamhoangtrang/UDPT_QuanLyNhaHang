...
if (isset($_POST['send'])) {
   ...
   $select_message = $db->getConnection('content')->prepare("SELECT * FROM `messages` WHERE name = ? AND email = ? AND number = ? AND message = ?");
   ...
   if ($select_message->rowCount() > 0) {
      ...
      $insert_message = $db->getConnection('content')->prepare("INSERT INTO `messages`(user_id, name, email, number, message) VALUES(?,?,?,?,?)");
