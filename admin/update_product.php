...
if (isset($_POST['update'])) {
   ...
   $update_product = $db->getConnection('product')->prepare("UPDATE `products` SET name = ?, category = ?, price = ? WHERE id = ?");
   ...
   if (!empty($image)) {
      if ($image_size > 2000000) {
         ...
         $update_image = $db->getConnection('product')->prepare("UPDATE `products` SET image = ? WHERE id = ?");
...
<body>
   ...
   <section class="update-product">
      ...
      $show_products = $db->getConnection('product')->prepare("SELECT * FROM `products` WHERE id = ?");
